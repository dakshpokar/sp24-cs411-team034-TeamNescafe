...
const Properties = () => {
	...
	const [highlyRatedPropertyIds, setHighlyRatedPropertyIds] =
		useState([]);
	...
	const fetchHighlyRatedProperties = useCallback(
		async (flag, areaMin, areaMax, priceMin, priceMax) => {
			try {
				...
				const response =
					await apiService.advancedPropertiesFilter(params);
				...
				console.error(
					'Error fetching highly rated property IDs:',
					error
				);
			}
		},
			...
			.then((propertiesData) => {
				...
				if (highlyRatedPropertyIds.length > 0) {
					...
					filtered = propertiesData.filter((property) =>
						highlyRatedPropertyIds.includes(
